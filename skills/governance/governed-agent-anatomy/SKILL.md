...
- **DAG Cognition Model:** `references/dag-cognition-model.md` — tri-layer architecture extending Memory/State/Truth primitives. Execution DAG (Layer 1) + Constitutional Ledger (Layer 2) + Semantic Index (Layer 3). Branch=lease for subagents, rewind=pointer shift + reversion receipt. Forged 2026-07-20 from Reddit git-as-DB insight + arifOS constitutional synthesis.
...
- **Institutional Intelligence Synthesis:** `references/institutional-intelligence-synthesis.md` — literature research across Ostrom (8 CDPs), Crossan (4I), March (exploration-exploitation), Hila (reflective knowledge), North, ContextGraph, Mnemoria, MultiA, PNAS (2025). What makes an institution intelligent at civilization scale. AAA crosswalk included. Forged 2026-07-20.
- **PRL Architecture Blueprint:** `references/prl-architecture-blueprint.md` — Precedent Retrieval Layer design: dual-gate architecture (cosine τ ≥ 0.95 + blast_radius payload filter + Ω₀ trigger), Phase 1 forge tasks, autoimmune prevention. Ratified by Arif 2026-07-20. IN FORGE — not yet deployed.
