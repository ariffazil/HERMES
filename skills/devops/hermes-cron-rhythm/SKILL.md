...
- **Updated:** 2026-07-26 — Fixed `hermes cron update` → `hermes cron edit` in Routing Audit Protocol fix command and pitfalls section (CLI uses `edit`, not `update`). Added `base_url`, `context_from`, `skills`, `skill`, and `no_agent` fields to Field Schema in `references/model-drift-mechanism.md` with a proper field table. All discovered by direct `jobs.json` inspection.
