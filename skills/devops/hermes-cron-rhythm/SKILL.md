...
- **The drift guard compares at fire time, not continuously.** Even if a job was created months ago with a now-retired model, it won't be blocked until its next scheduled tick. A job that last ran successfully yesterday may fail today because the global model changed overnight.
