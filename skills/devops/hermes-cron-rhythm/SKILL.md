...
- **Script path resolution: `script` field resolves relative to `~/.hermes/scripts/`.** For `no_agent: true` jobs, Hermes resolves the `script` field relative to `~/.hermes/scripts/`. A script existing at `/root/paper_trading/morning_scan.sh` does NOT mean `script: "paper_trading/morning_scan.sh"` works — Hermes looks at `~/.hermes/scripts/paper_trading/morning_scan.sh`. Fix: copy the script into the Hermes scripts directory tree (preserving subdirectory structure if needed), or use an absolute path. **SOT-copy pattern (PROVEN 2026-08-03):** when the canonical script lives in a version-controlled SOT directory like `/root/HERMES/scripts/zen/` but cron can't find it, use a symlink: `ln -sf /root/HERMES/scripts/zen/<script> /root/.hermes/scripts/<script>`. Cron follows symlinks correctly. Proven for 4 failing scripts: watchdog_of_watchdogs.py, evening_zen_brief.sh, git_to_vault.py, temporal_anomaly_detector.py — all errored "Script not found" until symlinked. **Proven 2026-07-25:** `🧠 Paper Trading Morning Analysis` errored with `Script not found: /root/HERMES/scripts/paper_trading/morning_scan.sh`. Fixed by `mkdir -p ~/.hermes/scripts/paper_trading && cp /root/paper_trading/morning_scan.sh $_`.
...
- **Stale key in gateway after rotation — ALL LLM cron jobs fail with 401.** After rotating an API key in kunci-mas.flat.env, the running gateway process still holds the OLD key in its process environment. The `EnvironmentFile` on disk is correct, but the process was launched before rotation and hasn't reloaded. **Symptoms:** all `no_agent` jobs pass, all LLM jobs fail with HTTP 401 on the primary provider → cascade fallback (F1 401, F2 404) → ALL LLM cron jobs error. The interactive CLI session works because it picks up the new key from the shell. **Diagnosis:** compare `printenv QWEN_HERMES_API_KEY` (shell) vs `/proc/<gateway_pid>/environ | tr '\0' '\n' | grep QWEN_` (process) — different key prefixes → stale. **Fix:** `systemctl restart hermes-gateway-api.service hermes-real-bridge.service` — these have `EnvironmentFile=/root/.secrets/kunci-mas.flat.env` and will pick up the new key on restart. Wait for re-init ("kanban dispatcher: embedded in gateway") before running jobs manually. **Proven 2026-08-03:** key rotated, gateway started before rotation, 13+ LLM cron jobs failed with cascade 401→401→404 until restart. After restart: all lanes green.

- **`cron.provider: ''` (empty string) kills ALL LLM cron jobs with 401 — even when API keys are valid.** The `cron.provider` field in `~/.hermes/config.yaml` is the default provider resolver for cron children. When empty (or unset), cron child processes cannot resolve ANY provider key — `curl` to the same key works fine from the shell, but every LLM cron job fails with `RuntimeError: HTTP 401: Invalid API Key` regardless of the per-job provider/model pin. `no_agent: true` script jobs are immune. **Masks as key-rotation or provider outage — always check `cron.provider` first when ALL LLM jobs fail simultaneously.** **Fix:** `hermes config set cron.provider <working_provider>` — one line restores all LLM cron jobs. Individual jobs still need their own provider/model pins to actually work (see next pitfall). **Proven 2026-08-03:** 13+ LLM cron jobs hitting 401 across 3 different providers; `cron.provider: ''` was the root cause; `hermes config set cron.provider deepseek` resolved key access for all jobs at once.

- **MCP tool overload — 128+ tools injected into LLM cron jobs even with `enabled_toolsets` filtered.** When MCP bridges are connected (aforge, arifos, geox, wealth, well, fed, github etc.), ALL their tools count toward the provider's per-request tool limit — DASHSCOPE/DeepSeek caps at 128. `enabled_toolsets: ["terminal"]` filters Hermes native tools but does NOT prevent MCP tool injection. **Symptoms:** `HTTP 400: 'tools': maximum number of items is 128` even with `enabled_toolsets` set. **Workaround:** switch provider to one with higher tool limits (MiniMax, Groq) for LLM cron jobs. Permanent fix requires MCP-layer tool filtering. **Proven 2026-08-03:** nightly-seal job hit 128-tool wall after `cron.provider` fix restored key access; switching from deepseek/dashscope to groq/llama-3.3-70b worked but groq API key access needed gateway restart.

After killing an orphan gateway (or any gateway restart), the surviving gateway enters a **180s drain timeout** (`gateway.log`: "Gateway drain timed out after 180s ... interrupting remaining work"). A manual `cronjob(action='run')` triggered in that window is torn down and surfaces as `RuntimeError: Connection error` — a self-inflicted transient, NOT a cron/provider defect. Repeated manual runs while the gateway re-inits produce a chain of identical failure alerts. **Fix:** after a gateway restart, wait for full re-init (gateway.log shows "kanban dispatcher: embedded in gateway") before running jobs manually; let scheduled ticks fire naturally for a clean repro. **Proven 2026-08-02:** repeated evening-digest manual runs during drain each produced `Connection error` before the gateway returned at +180s.
