...
- **Spawn-brain / orphan gateway race.** `hermes gateway run --replace` can fail to kill the old gateway — two gateways run simultaneously, both `PPID=1` (init-owned). The stale gateway (from a previous start, often systemd-spawned) has **empty environment** (no QWEN keys, no LLM credentials). Its cron children inherit nothing → every LLM call fails with "Connection error" or "401". Meanwhile, the authoritative gateway (declared in `~/.hermes/gateway.lock`) works fine for direct curls because those run in the current shell (which has vault.env sourced). **Detection:** `ps aux | grep gateway` shows two PIDs. Check `/proc/<pid>/environ` for each — the stale one has 21 vars (no QWEN_*), the real one has 40+ vars. `cat ~/.hermes/gateway.lock | python3 -c "import sys,json;print(json.load(sys.stdin)['pid'])"` tells you which PID is authoritative. **Fix:** `kill -9 <stale_pid>`. **Why SIGTERM didn't work:** the stale gateway's signal handler may be stuck in a drain loop (seen: 180s drain timeout). SIGKILL is safe — it's confirmed orphan (not the lock holder, no keys, no connections). **Proven 2026-08-02:** Gateway 2682212 (Aug 1, empty env) ran alongside 513083 (Aug 2, all keys). Cron jobs from 2682212 produced "Connection error" for every LLM call. Killing 2682212 with SIGKILL resolved all cron failures. Also: manual `cronjob(action='run')` during gateway drain/restart hits the 180s timeout → surfaces as "Connection error" even though it's a timing artifact. Wait 20s after kill before re-testing.

- **Caddy duplicate matcher blocks config reload.** Two matchers with the same name (`@name path /foo*` then `@name header X-Bar ...`) — Caddy silently uses the second, overwriting the first. Running config is fine (loaded before the error appeared), but any `caddy validate` or `systemctl reload caddy` fails → config changes are blocked until fixed. **Pattern:** intent was path AND header must match, but Caddy reads same-name matchers as overwrites, not AND. **Fix:** block syntax `@name { path /foo*; header X-Bar "value" }`. **Proven 2026-08-04:** `@static_client_id_mcp` defined twice on lines 1141-1142 of `/etc/caddy/Caddyfile` — path guard silently dropped. `caddy validate --config /etc/caddy/Caddyfile` fails; `caddy validate` without config path also fails (different reason: no default file). Both need explicit `--config /etc/caddy/Caddyfile`.

- **Verify probe claims before acting.** Autonomous heartbeat probes can report false positives — "API unreachable/blocked" when the real issue is wrong URL format in the probe script. Always verify against the actual endpoint before escalating. **Proven 2026-08-04:** Probe reported "Telegram API blocked from VPS" but `curl getMe` returned 200 live. The probe used a malformed URL. Cross-check: `source /root/.secrets/vault.env && curl -s "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/getMe"`.

- **Detached process ≠ managed process.** A service running as a bare `python3 /path/to/server.py`  process (started manually, no systemd unit) has zero auto-restart protection. If it crashes or the VPS reboots, it stays dead. Symptoms: process shows as running with `ps aux`, health endpoint responds, all fixes applied, but `systemctl is-active` returns `inactive` for both the expected unit name AND any plausible variants. Detection: `ps aux | grep <name>` shows the process; `systemctl list-units | grep <name>` shows nothing. Fix: create a systemd unit (Type=simple, Restart=always) and `systemctl enable --now`. Proven 2026-07-26: FLAME (:18901) ran as pid 1842392 since Jul25 with all P0/P1 fixes applied — but no systemd unit existed. A VPS reboot would lose it permanently.
