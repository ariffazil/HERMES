...
13. **Backend scripts have the same repo-vs-deployed split as static files** (PROVEN 2026-08-04, gold-api). Commodity API servers on arif-fazil.com (`/var/www/html/<surface>/api/server.js`) read Python via `path.join(__dirname, 'fetch_<surface>.py')`. The repo copy lives at `/root/arif-fazil.com/sites/arif-fazil.com/public/<surface>/api/fetch_<surface>.py`. Fixing the repo file does NOT update the served file — verify with `diff` + `stat -c '%y'` and probe the live endpoint before declaring "fixed + restarted". A three-way split (repo / WEALTH-organ parallel copy / deployed webroot) compounds the problem. See `deployment-claim-verification` pitfall #50 and `references/arif-fazil-com-three-way-paths.md` for the full topology + detection recipe + restart sequence.
