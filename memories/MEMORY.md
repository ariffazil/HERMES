...
§
Arif: observation vs interpretation MUST be explicit. Label each; don't fuse.
§
...
§
Bot routing: ASI💃(Hermes/Flash)=main, 🦞AGI(OpenClaw/Pro)=AAA, 🔥FORGE(OpenCode)=Arif DM. 5 agents: 333/555/888/antigravity/openclaw. A2A via AAA aaa.arif-fazil.com/a2a/openclaw; EMD gate blocks unauthed delegation.
§
Arif BENCI token/cost tracking — 'bukan casino'. No token counting, budgets, or API costs in proposals. Enforcement via state-based mechanisms (Ghost JSON hash, reasoning cycles, organ health).
§
...
§
Demucs 4.1 installed via pip install demucs --break-system-packages. Output at /tmp/separated/htdemucs/<filename>/. Works on CPU (~24s/30s). AI cover pipeline script: /root/HERMES/scripts/kaparinyo-pipeline.sh (yt-dlp → Demucs → RVC → FFmpeg).
§
